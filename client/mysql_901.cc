...
static int com_extra(String *buffer MY_ATTRIBUTE((unused)), char *line) {
    ...
    // 명령어 정의 - 배열 범위 체크 추가
    if (line && strlen(line) >= 4) {  // 최소 4글자 필요 (\\ + 2글자 명령어)
        ...
        return put_info("Invalid command format\n", INFO_ERROR, 0);
    }

    while (line && *line && my_isspace(charset_info, *line)) line++;
    if (!line || !(param = strchr(line, ' '))){
        // No parameters
    }else{
        while (param && *param && my_isspace(charset_info, *param)) param++;
        if (param && *param) {
            end = strmake(object_name, param, sizeof(object_name) - 1);
            if (end && end > object_name){
    ...
    if(user_command[0]=='t' && user_command[1]=='c' && isdigit(user_command[2])==true){
	    ...
	    if (mysql_query(&mysql_handle, "SELECT IFNULL(DATABASE(), 'none')") != 0) {
	        puts("Failed to get current database\n");
	        return 0;
	    }
        ...
        if (!row || !row[0]) {
            puts("No database information\n");
            mysql_free_result(result);
            return 0;
        }
        
        if (strlen(row[0]) < sizeof(chosen_database)) {
        ...
        char cmd3[4]={user_command[2], user_command[3], '\0'}; //{테이블 1의 자리, 테이블 10의 자리}

        vertical =true;
        if (strlen(cmd1) + strlen(chosen_database) + strlen(cmd2) + strlen(cmd3) + 2 < sizeof(cmd1)) {
            strcat(cmd1, chosen_database);
            strcat(cmd1, cmd2);
            strcat(cmd1, cmd3);
            strcat(cmd1, ";");

            if (mysql_query(&mysql_handle, cmd1) != 0) {
                puts("Failed to query table information\n");
                vertical = oldvertical;
                return 0;
            }
            ...
            result = mysql_use_result(&mysql_handle);
            if (!result) {
                puts("Failed to get table result\n");
                vertical = oldvertical;
                return 0;
            }
            ...
            if (row[0] && strlen(row[0]) < sizeof(chosen_table)) {
    ...
    //mysql> \\dd{number}
        ...
        MYSQL_RES *result=nullptr;
        //MYSQL_FIELD *field;
        char cmd1[1000]="SELECT A.schema_name FROM (SELECT row_number()over(order by schema_name) AS number, schema_name FROM INFORMATION_SCHEMA.SCHEMATA) AS A WHERE A.number = ";
        char cmd2[4]={user_command[2], user_command[3], '\0'}; //{데이터베이스, 세미콜론}
        char chosen_database[100]="";

        if (strlen(cmd1) + strlen(cmd2) + 2 < sizeof(cmd1)) {
            strcat(cmd1, cmd2);
            strcat(cmd1, ";");

            if (mysql_query(&mysql_handle, cmd1) != 0) {
                puts("Failed to query database information\n");
                return 0;
            }
            ...
            if (row[0] && strlen(row[0]) < sizeof(chosen_database)) {
    ...
    //mysql> \\dds
        ...
        if (isdigit(user_command[3])==true){            
            ...
            char cmd2[4]={user_command[3], user_command[4], '\0'}; //{테이블 1의 자리, 테이블 10의 자리}
            ...
            if (strlen(cmd1) + strlen(cmd2) + 2 < sizeof(cmd1)) {
                ...
                if (mysql_query(&mysql_handle, cmd1) != 0) {
                    puts("Failed to query database size information\n");
                    return 0;
                }
    ...
    //mysql> \\uuc
        ...
        if (isdigit(user_command[3])==true){
            ...
            char cmd2[4]={user_command[3], user_command[4], '\0'}; //{유저 1의 자, 유저 10의 자리}
            ...
            if (strlen(cmd1) + strlen(cmd2) + 2 < sizeof(cmd1)) {
                ...
                if (mysql_query(&mysql_handle, cmd1) != 0) {
                    puts("Failed to query user information\n");
                    return 0;
                }
                ...
                if (row[1] && strlen(row[1]) < sizeof(chosen_user)) {
                    strcat(chosen_user, row[1]); // 선택한 유저 받아오기
                }
                if (row[2] && strlen(row[2]) < sizeof(chosen_host)) {
                    strcat(chosen_host, row[2]); // 선택한 호스트 받아오기
                }
    ...
    //mysql> \\qq{limit}
    else if(user_command[0]=='q' && user_command[1]=='q' && isdigit(user_command[2])==true){
		if(user_command[2]=='0'){
			return 0;
		}
        if(isdigit(user_command[3])==false){
            glob_buffer.append( STRING_WITH_LEN("  SELECT * FROM ") );
       	    glob_buffer.append( object_name, strlen(object_name) );
            glob_buffer.append( STRING_WITH_LEN("  LIMIT ") );
            glob_buffer.append( &user_command[2], 1 );
        }
        else if (isdigit(user_command[3])==true){
            glob_buffer.append( STRING_WITH_LEN("  SELECT * FROM ") );
       	    glob_buffer.append( object_name, strlen(object_name) );
            glob_buffer.append( STRING_WITH_LEN("  LIMIT ") );
            glob_buffer.append( &user_command[2], 1 );
